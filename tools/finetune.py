...
def parse_args():
    ...
    parser.add_argument("--shared-file", type=str, default="/groups/gaa50006/shlok/pytorch.shared")
...
def main(single_trial = None):
    ...
    logging.info("=> loading state-dict of pre-trained model from {}"
    		.format(str(config.FINETUNE.MODEL_FILE)))
    pre_state_dict = torch.load(config.FINETUNE.MODEL_FILE)
    model.load_state_dict(pre_state_dict)
    logging.info("=> state-dict loaded successfully.")

    if model.head.weight.shape[0] != config.MODEL.NUM_CLASSES:
        model = torch.nn.Sequential(
            model,
            torch.nn.Linear(model.head.weight.shape[0], config.MODEL.NUM_CLASSES, bias= True)
        )
    else:
        pass
...
if __name__ == '__main__':
    ...
    if args.optuna:
        group_dir = '/groups/gaa50006/shlok/job_logs'
